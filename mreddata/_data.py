...
class Hdf5Data:
	...
	def getNormFactor(self):
		'''
		Returns the normalization factor for MRED data by looking for the HDF5 file attributes
		"nIons" and "gfu". Normalization factor is given by 1 / (gfu * nIons). 
		'''
		try:
			if not options.raw:
				return 1./(self.attrs['gfu'][0] * self.attrs['nIons'][0])
		except Exception as e:
			print("Error in getNormFactor()")
			print(e)
			try:
				return 1./(self.attrs['nIons'][0])
			except:
	...
	def getHistogram(self, histName=None, diff = options.diff):
		'''
		returns only one histogram labelled by @histName for a given file. Defaults to the first 
		histogram if no @histName passed 
		'''
		try:
	...
	def getHistograms(self, histograms=None):
		'''
	 	returns a dictionary of histogram DataFrames with the names as keys. Adds to the 
	 	histogramTable attribute if the -k option flag is selected at runtime	
	...
	def getTotalDose(self, histName=None, includeUnderflow=False):
		histogram = self.getHistogram(histName=histName, diff=True)	
		startIndex = 0 if includeUnderflow else 1
